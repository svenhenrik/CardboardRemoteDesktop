...
abstract class AbstractBitmapData {
	...
	AbstractBitmapData( RfbProto p, VncView c)
	{
		rfb=p;
		vncView = c;
	...
	{
		if (vncView.connection.getUseLocalCursor())
		{
			if (drawable==null)
				drawable = createDrawable();
			drawable.setCursorRect(vncView.mouseX, vncView.mouseY);
			vncView.invalidate(drawable.cursorRect);
		}
	}
	...
	{
		double scale = 0.75;
		int displayWidth = vncView.getWidth();
		int displayHeight = vncView.getHeight();
