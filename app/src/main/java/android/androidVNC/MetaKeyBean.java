...
class MetaKeyBean extends AbstractMetaKeyBean implements Comparable<MetaKeyBean> {
	...
	static {
		...
		allKeys.add(new MetaKeyBase(VncView.MOUSE_BUTTON_LEFT,"Mouse Left"));
		allKeys.add(new MetaKeyBase(VncView.MOUSE_BUTTON_MIDDLE,"Mouse Middle"));
		allKeys.add(new MetaKeyBase(VncView.MOUSE_BUTTON_RIGHT,"Mouse Right"));
		allKeys.add(new MetaKeyBase(VncView.MOUSE_BUTTON_SCROLL_DOWN, "Mouse Scroll Down"));
		allKeys.add(new MetaKeyBase(VncView.MOUSE_BUTTON_SCROLL_UP, "Mouse Scroll Up"));
		...
		keyCtrlAltDel = new MetaKeyBean(0, VncView.CTRL_MASK| VncView.ALT_MASK,keysByKeyCode.get(KeyEvent.KEYCODE_DEL));
		...
		if (_regenDesc)
		{
			synchronized(this)
			{
				if (_regenDesc)
				{
					...
					if (0 != (meta & VncView.SHIFT_MASK))
					{
						sb.append("Shift");
					}
					if (0 != (meta & VncView.CTRL_MASK))
					{
						if (sb.length()>0)
							sb.append('-');
						sb.append("Ctrl");
					}
					if (0 != (meta & VncView.ALT_MASK))
					{
						if (sb.length()>0)
							sb.append('-');
						sb.append("Alt");
