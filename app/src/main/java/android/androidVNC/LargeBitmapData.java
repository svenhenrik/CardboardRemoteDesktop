...
class LargeBitmapData extends AbstractBitmapData {
	 ...
	 */
	LargeBitmapData(RfbProto p, VncView c, int displayWidth, int displayHeight, int capacity)
	{
		super(p,c);
	...
	synchronized void scrollChanged(int newx, int newy) {
		...
		int visibleWidth = vncView.getVisibleWidth();
		int visibleHeight = vncView.getVisibleHeight();
		if (newx - xoffset < 0 )
		{
