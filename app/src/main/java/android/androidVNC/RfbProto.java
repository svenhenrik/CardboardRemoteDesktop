...
class RfbProto {
  ...
  */
  ...
  void writeCtrlAltDel() throws IOException {
	  ...
	  final int CTRLALT = VncView.CTRL_MASK | VncView.ALT_MASK;
	  try {
		  // Press
		  eventBufLen = 0;
  ...
  void writeModifierKeyEvents(int newModifiers) {
    if ((newModifiers & VncView.CTRL_MASK) != (oldModifiers & VncView.CTRL_MASK))
      writeKeyEvent(0xffe3, (newModifiers & VncView.CTRL_MASK) != 0);

    if ((newModifiers & VncView.SHIFT_MASK) != (oldModifiers & VncView.SHIFT_MASK))
      writeKeyEvent(0xffe1, (newModifiers & VncView.SHIFT_MASK) != 0);

    if ((newModifiers & VncView.META_MASK) != (oldModifiers & VncView.META_MASK))
      writeKeyEvent(0xffe7, (newModifiers & VncView.META_MASK) != 0);

    if ((newModifiers & VncView.ALT_MASK) != (oldModifiers & VncView.ALT_MASK))
      writeKeyEvent(0xffe9, (newModifiers & VncView.ALT_MASK) != 0);
