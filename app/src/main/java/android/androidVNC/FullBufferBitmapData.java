...
class FullBufferBitmapData extends AbstractBitmapData {
	...
	class Drawable extends AbstractBitmapDrawable {
		...
		public void draw(Canvas canvas) {
			if (vncView.getScaleType() == ImageView.ScaleType.FIT_CENTER)
			{
			...
			else
			{
				...
				{
				*/
					int drawWidth = vncView.getVisibleWidth();
					if (drawWidth + xo > data.framebufferwidth)
						drawWidth = data.framebufferwidth - xo;
					int drawHeight = vncView.getVisibleHeight();
			...
			if(data.vncView.connection.getUseLocalCursor())
			{
				setCursorRect(data.vncView.mouseX, data.vncView.mouseY);
	 ...
	 * @param c
	 */
	public FullBufferBitmapData(RfbProto p, VncView c, int capacity) {
		super(p, c);
